...
const createMiddleware = function (middleware) {
    ...
    APP_PATH = getProjectAppPath(projectRootPath);
    helper.mkDir(APP_PATH + path.sep + 'middleware');
    copyFile('middleware' + path.sep + 'middleware.ts', APP_PATH + path.sep + 'middleware' + path.sep + middleware + 'Mid.ts', {
    ...
    console.log();
    console.log('  list: [..., "' + middleware + 'Mid"] //加载中间件' + middleware);
...
const createModel = function (model) {
    ...
    if (orm == 'typeorm') {
        ...
        console.log('  please modify /app/config/middlewate.ts file:');
        console.log();
        console.log('  list: [..., "TypeormMid"] //加载中间件' + middleware);
        console.log('  config: { //中间件配置 ');
...
commander.option('-o, --orm', 'used orm module (thinkorm, typeorm), default is thinkorm, used in `think model` command');
