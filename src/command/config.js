/*
 * @Description:
 * @Usage:
 * @Author: richen
 * @Date: 2020-12-22 17:29:34
 * @LastEditTime: 2023-12-24 15:41:12
 */

module.exports = {
  TEMPLATE_NAME: 'koatty_template',
  TEMPLATE_URL: 'https://githubfast.com/Koatty/koatty_template.git#main',
  CLI_TEMPLATE_NAME: 'koatty_template_cli',
  CLI_TEMPLATE_URL: 'https://githubfast.com/Koatty/koatty_template_cli.git#main',
  COM_TEMPLATE_NAME: 'koatty_template_component',
  COM_TEMPLATE_URL: 'https://githubfast.com/Koatty/koatty_template_component.git#main',

  GRPC_IMPORT: "//_IMPORT_LIST Important! Do not delete this line",
  GRPC_METHOD: "//_METHOD_LIST Important! Do not delete this line",

  LOGO: `

┬┌─┌─┐┌─┐┌┬┐┌┬┐┬ ┬
├┴┐│ │├─┤ │  │ └┬┘
┴ ┴└─┘┴ ┴ ┴  ┴  ┴ 
-------------------------------------------
https://github.com/koatty
    `
};